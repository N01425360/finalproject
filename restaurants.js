...
const users = require('./restaurants.json');
...
// Route for getting all data working with proper CSS
 app.get('/allData', function(req,res)
 {
     res.render('alldata',{DATA : users});
 });
...
//sorting the data with page, per page and Borough working
app.post('/sort', async (req, res) => {    
    try {
       const filter = req.body.query;
       let where = {};
       if (filter.borough) {
           where.borough = { $regex: filter.borough, $options: "i" }
       }
       let query = restaurant.find(where);
       const page = parseInt(req.body.page) || 1;
       const pageSize = parseInt(req.body.limit) || 10;
       const skip = (page - 1) * pageSize;
       const total = await restaurant.countDocuments(where);
